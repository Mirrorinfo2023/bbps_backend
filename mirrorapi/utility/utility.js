...
function DataDecrypt(data) {
  try {
    ...
    // Step 3: Replace characters back
    const decryptedKey = buffer
      .split("")
      .map((char) => reverseSubstitutionMap[char] || char)
      .join("");

    // Step 4: Clean string (replace single quotes)
    const decryptString = decryptedKey.replace(/'/g, '"').trim();

    // Step 5: Try to parse JSON safely
    try {
      return JSON.parse(decryptString);
    } catch (jsonErr) {
      console.warn("⚠️ DataDecrypt Warning: Decrypted string is not valid JSON.");
      console.log("🔹 Raw decrypted string (first 200 chars):", decryptString.slice(0, 200));
      return decryptString; // return as raw string instead of crashing
    }
  } catch (err) {
    console.error("❌ DataDecrypt Error:", err.message);
    throw new Error("Decryption failed or invalid base64 input");
  }
}
...
function setKeySecret(secret) {
  if (!secret) {
    throw new Error('setKeySecret: secret is required');
  }
  // sha256 => 32 bytes
  const hashedKey = crypto.createHash('sha256').update(String(secret), 'utf8').digest();
  // ensure Buffer(32)
  if (!Buffer.isBuffer(hashedKey) || hashedKey.length < 32) {
    throw new Error('setKeySecret: failed to generate 32-byte key');
  }
  return hashedKey.slice(0, 32); // 32 bytes for aes-256
}

/**
 * Encrypt JS object (or string) -> base64 using AES-256-ECB.
 * Returns base64 string, or null on failure.
 */
function clientDataEncrypt(data, SECRET_KEY) {
  try {

    if (!SECRET_KEY) {
      console.error('clientDataEncrypt: SECRET_KEY is missing');
      return null;
    }
    if (typeof data === 'undefined' || data === null) {
      console.error('clientDataEncrypt: data is missing or undefined');
      return null;
    }
    ...
    // derive 32-byte key
    const key = setKeySecret(SECRET_KEY);
    ...
    console.error('clientDataEncrypt failed:', err && err.message ? err.message : err);
    return null;
  }
}

/**
 * Decrypt base64 -> JSON object using AES-256-ECB and provided SECRET_KEY.
 * Returns parsed object or null on failure.
 */
function clientDataDecrypt(encryptedData, SECRET_KEY) {
  try {
    if (!SECRET_KEY) {
      console.error('clientDataDecrypt: SECRET_KEY is missing');
      return null;
    }
    if (!encryptedData) {
      console.error('clientDataDecrypt: encryptedData is missing');
      return null;
    }
    ...
    // If decrypted is a JSON string -> parse, else return raw string
    try {
    ...
    console.error('clientDataDecrypt failed:', err && err.message ? err.message : err);
    return null;
  }
}
